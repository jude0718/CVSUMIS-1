    ...
    public function RecognitionAndAwardCSV(Request $request)
{
    ...
    $filename = $directory . '/Recognition_Award_List.csv';

    $fp = fopen($filename, "w+");

    fputcsv($fp, ['ADDED BY', 'RECOGNITION', 'GRANTING AGENCY', 'START YEAR', 'END YEAR']);

    $query = AwardsHeader::latest()->with('created_by_dtls');
    ...
    if ($request->has('year') && !empty($request->year)) {
        $query->whereYear('start_year', $request->year);
    }
    ...
    foreach ($data as $row) {
        fputcsv($fp, [
            ...
            $row->award,
            $row->granting_agency,
            date('Y', strtotime($row->start_year)),
            date('Y', strtotime($row->end_year))
        ]);
    }
    ...
    return response()->download($filename, 'Recognition_Award_List.csv', $headers)->deleteFileAfterSend(true);
}
    ...
    public function fetchAwardData(){
        ...
        if(Auth::user()->position == 1 || Auth::user()->position == 5){
            ...
            $data = AwardsHeader::whereHas('created_by_dtls', function ($query) {
